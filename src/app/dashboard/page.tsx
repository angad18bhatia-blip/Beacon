  ...
  const tiles = [
    {
      ...
      color: "--accent2",
      href: "/professors?status=NEW",
    },
    {
      label: "Drafted",
      count: professors.filter((p) => p.status === "DRAFTED").length,
      color: "--amber",
      href: "/professors?status=DRAFTED",
    },
    {
      label: "Approved",
      count: professors.filter((p) => p.status === "APPROVED").length,
      color: "--accent",
      href: "/professors?status=APPROVED",
    },
    {
      label: "Sent",
      count: professors.filter((p) => p.status === "SENT").length,
      color: "--teal",
      href: "/professors?status=SENT",
    },
    {
      label: "Replied",
      count: professors.filter((p) => p.hasReply).length,
      color: "--pink",
      href: "/professors?replied=1",
    },
  ];

  const sent = professors.filter((p) => p.status === "SENT");
  const repliedCount = sent.filter((p) => p.hasReply).length;
  const overallRate =
    sent.length > 0 ? Math.round((repliedCount / sent.length) * 100) : 0;
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            ...
            {sent.length === 0
              ? "—"
              : `${overallRate}% (${repliedCount}/${sent.length} sent)`}
          </p>
        </div>
        {sent.length > 0 && <CheckRepliesButton />}
      </div>
      ...
      {templateRows.length === 0 ? (
        ...
        <>
          ...
          <p className="mt-6 text-xs text-zinc-500">
            Reply rates are a signal, not a guarantee &mdash; small sample
            sizes (a handful of emails per prompt) will bounce around a lot.
            Don&apos;t over-index on this until you&apos;ve sent enough to
            trust it.
          </p>
        </>
      )}
